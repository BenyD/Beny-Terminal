...
export const LennyFace = () => {
  ...
  return (
    <div
      className={`not-sr-only absolute right-4 top-1/2 hidden -translate-y-1/2 select-none items-center transition-transform duration-300 lg:flex ${
        visible ? '-translate-y-1/2 opacity-100' : 'translate-y-full opacity-0'
      }`}
      onClick={() => setClicked(!clicked)}
      onDoubleClick={(e) => e.stopPropagation()}
    >
      {clicked && <p className="mb-1.5 mr-1 text-xl">♡</p>}
      <div className="flex items-center space-x-1.5">
        <Eye />
        <span
          className={`${clicked ? 'mt-5 rotate-180' : 'mt-2.5'} transition-all`}
        >
          ‿
        </span>
        <Eye />
...
const Eye = () => {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      const x =
        50 + ((Math.cos(angle) * clampedDistance) / (rect.width / 2)) * 45;
      const y =
        50 + ((Math.sin(angle) * clampedDistance) / (rect.height / 2)) * 45;
