...
export function TicTacToe({ data, onGameUpdate }: TicTacToeProps) {
  ...
  const renderSquare = (index: number) => {
    return (
      <button
        key={index}
        className={`ttt-square flex h-16 w-16 items-center justify-center border border-gray-600 text-2xl font-bold ${
          board[index] === 'X'
            ? 'text-blue-400'
            : board[index] === 'O'
              ? 'text-red-400'
  ...
  return (
    <div className="tic-tac-toe-game flex flex-col items-center">
      <div className="status mb-4 text-center">
        <div
          className={`py-2 text-center ${winner ? (winner === 'X' ? 'text-blue-400' : winner === 'O' ? 'text-red-400' : 'text-yellow-400') : 'text-green-400'}`}
        >
          {winner
            ? winner === 'Draw'
              ? 'Game ended in a draw!'
              : `Winner: ${winner}`
            : 'Your turn! X goes first.'}
        </div>

        <div className="player-indicators mb-2 flex justify-center gap-4">
          <div
            className={`rounded px-3 py-1 ${xIsNext && !winner ? 'bg-blue-900 text-blue-300' : 'bg-gray-800 text-gray-400'}`}
          >
            You: X
          </div>
          <div
            className={`rounded px-3 py-1 ${!xIsNext && !winner ? 'bg-red-900 text-red-300' : 'bg-gray-800 text-gray-400'}`}
          >
            Computer: O
          </div>
        </div>
      ...
      <div className="board grid grid-cols-3 gap-1 bg-gray-700">
      ...
      {winner && (
        <button
          onClick={resetGame}
          className="mt-4 rounded bg-green-700 px-4 py-2 text-white hover:bg-green-600"
        >
          Play Again
        </button>
      )}
