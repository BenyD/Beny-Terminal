...
export function TerminalInput() {
  const { input, setInput, executeCommand, commandHistory, historyIndex, setHistoryIndex, terminal, clearTerminal, inputRef, focusInput } = useTerminal()
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === 'Enter') {
      ...
      if (historyIndex > 0) {
        const newIndex = historyIndex - 1
        setHistoryIndex(newIndex)
        setInput(newIndex === 0 ? '' : commandHistory[commandHistory.length - newIndex])
      }
    } else if (e.key === 'Tab') {
      e.preventDefault()
      handleTabCompletion()
  ...
  const handleTabCompletion = () => {
    ...
    const filtered = basicCommands.filter(c => c.command.startsWith(cmd.toLowerCase()))
  ...
  return (
    <div onClick={handleContainerClick} className='terminal-input relative'>
      <div className='flex items-center'>
        ...
        />
        <span className='animate-blink h-4 w-2 bg-gray-400'></span>
      </div>

      {/* Autocomplete suggestions */}
      {showSuggestions && suggestions.length > 0 && (
        <div className='absolute bottom-full left-0 z-10 w-full max-h-48 overflow-y-auto bg-gray-900 border border-gray-700 rounded-md shadow-lg'>
          {suggestions.map((suggestion, idx) => (
            <div
              key={suggestion.command}
              className={`px-3 py-1 flex justify-between ${idx === selectedSuggestion ? 'bg-gray-700' : ''}`}
              onClick={() => {
