      ...
      },
      {
        ...
        children: [
          { name: 'email.txt', type: 'file', content: 'hello@beny.one' },
          {
            name: 'social.txt',
            type: 'file',
            content: 'GitHub: https://github.com/BenyD\nTwitter: @beny_d',
          },
        ],
      },
...
export function TerminalProvider({ children }: { children: React.ReactNode }) {
    ...
    outputs: [
      {
        content: (
          <div className="welcome-message">
            <pre className="font-mono text-blue-400">{asciiArt.welcome}</pre>
            <p className="mt-2 text-gray-400">
              Welcome to beny.one terminal! Type{' '}
    ...
    const interval = setInterval(() => {
      setTerminal((prev) => ({
        ...
        systemStats: {
          cpu: Math.min(
            100,
            Math.max(5, prev.systemStats.cpu + (Math.random() * 10 - 5))
          ),
          memory: Math.min(
            100,
            Math.max(10, prev.systemStats.memory + (Math.random() * 6 - 3))
          ),
          network: Math.min(
            100,
            Math.max(1, prev.systemStats.network + (Math.random() * 8 - 4))
          ),
        },
      }));
    }, 1000);
      ...
      for (const segment of pathSegments) {
        if (segment === '..') {
          ...
          if (terminal.currentDirectory.length > 1) {
            return findFile(
              '/' + terminal.currentDirectory.slice(0, -1).join('/')
            );
          }
          continue;
        }

        const child: FileSystemItem | undefined = current?.children?.find(
    ...
    for (const segment of segments) {
      if (segment === '..') {
        ...
        if (valid) {
          ...
          addOutput({
            content: `cd: ${segment}: No such directory`,
            isError: true,
          });
          return;
        }
  ...
  const createTerminal = (
    title: string = `terminal-${Date.now().toString(36)}`
  ) => {
  ...
  const updateGameState = (
    ...
    setTerminal((prev) => {
      ...
      if (
        prev.gameState.activeGame === game &&
        JSON.stringify(prev.gameState.data) === JSON.stringify(data)
      ) {
        return prev;
      }
