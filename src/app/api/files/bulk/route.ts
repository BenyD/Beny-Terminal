  ...
  try {
    ...
    switch (action) {
      case 'delete':
        ...
        const { error: storageError } = await supabase.storage.from('assets').remove(fileIds)
        ...
        // Delete metadata
        const { error: metadataError } = await supabase.from('file_metadata').delete().in('file_name', fileIds)
      ...
      case 'move':
        if (!category) {
          return NextResponse.json({ error: 'Category is required for move operation' }, { status: 400 })
        }

        // Update category for selected files
        const { error: updateError } = await supabase.from('file_metadata').update({ category }).in('file_name', fileIds)

        if (updateError) {
          console.error('Error updating file categories:', updateError)
          return NextResponse.json({ error: 'Failed to update file categories' }, { status: 500 })
        }
