...
export async function GET() {
  ...
  if (ENV.UMAMI_API_KEY) {
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`API key request failed (${response.status}): ${errorText}`)
        throw new Error(`Failed to fetch from Umami API: ${response.status}`)
      }

      const data = await response.json()
      console.log('Umami API response:', JSON.stringify(data).substring(0, 200) + '...')

      // Extract pageviews from API response
      let pageViewsCount = null
      if (data.pageviews !== undefined) {
        pageViewsCount = typeof data.pageviews === 'number' ? data.pageviews : data.pageviews.value || data.pageviews
      }
  ...
  try {
    ...
    const data = await response.json()
    console.log('Share URL response:', JSON.stringify(data).substring(0, 200) + '...')
