...
type _ChartType = 'progress' | 'bar' | 'table';
type StatCategory = 'activity' | 'languages' | 'editors' | 'os';

const WAKATIME_ENDPOINTS = {
  CODING_ACTIVITY:
    'https://wakatime.com/share/@06952d26-d5c1-4a8e-bf7c-c72d88cf5a38/c989e85d-2a3f-4aa1-95b0-e024a99433eb.json',
  LANGUAGES:
    'https://wakatime.com/share/@06952d26-d5c1-4a8e-bf7c-c72d88cf5a38/8cd19e4b-9a31-4040-bf66-744208c3db3c.json',
  EDITORS:
    'https://wakatime.com/share/@06952d26-d5c1-4a8e-bf7c-c72d88cf5a38/fb10d5c3-1498-4122-8863-cdd759cb3f7f.json',
  OPERATING_SYSTEMS:
    'https://wakatime.com/share/@06952d26-d5c1-4a8e-bf7c-c72d88cf5a38/58af6750-01e9-4d03-b534-46a78a2b3a44.json',
};

type _GenericData = WakaTimeData['data'] | CodingActivityData['data'];
...
export default function StatsPage() {
  ...
  const [operatingSystems, setOperatingSystems] = useState<WakaTimeData | null>(
    null
  );
  const [activeCategory, setActiveCategory] =
    useState<StatCategory>('activity');
    ...
    const fetchData = async () => {
      try {
        ...
        const fetchWakaTimeData = async (url: string) => {
          try {
            const response = await fetch(
              `/api/wakatime?url=${encodeURIComponent(url)}`
            );
            if (!response.ok) {
        ...
        if (allFailed) {
          setHasError(true);
          setErrorMessage(
            'Unable to connect to WakaTime. Please try again later or check your embeddable chart links.'
          );
          return;
        }
        ...
        if (results[3].status === 'fulfilled')
          setOperatingSystems(results[3].value);
      } catch (error) {
        console.error('Error fetching WakaTime data:', error);
        setHasError(true);
        setErrorMessage(
          'Failed to load WakaTime data. Please check your console for details.'
        );
      } finally {
        setIsLoading(false);
      }
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto p-4">
      <h1 className="mb-6 font-sans text-2xl text-foreground/90">
        Coding Stats
      </h1>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center space-y-6 p-10 text-center text-foreground/50">
          <svg
          ...
          <div className="font-medium text-foreground/80">
            Error Loading WakaTime Data
          </div>
          <div className="max-w-md text-sm">{errorMessage}</div>
          <div className="mt-2 text-xs">
            Make sure your WakaTime embeddable links are valid and current.
            Visit{' '}
            <a
          ...
          <button
            ...
            className="mt-4 rounded-md bg-[#242424] px-4 py-2 text-foreground/80 transition-colors hover:bg-[#303030]"
          >
            Try Again
          </button>
        </div>
        ...
        <div className="flex flex-col gap-6">
          {/* Category Tabs */}
          <div className="flex overflow-hidden rounded-lg border border-border/10 bg-[#1A1A1A]">
            <button
          ...
          <div className="rounded-lg border border-border/10 bg-[#1A1A1A] p-4">
            <h2 className="mb-6 font-sans text-xl text-foreground/80">
              {getCategoryTitle()}
            </h2>
            {renderVisualization()}
          </div>
...
function ActivityChart({ data }: { data: CodingActivityData['data'] }) {
  if (!data || data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center space-y-4 p-10 text-center text-foreground/50">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="24"
          height="24"
        ...
        <div className="text-xs">
          WakaTime data will appear here once you start coding
        </div>
      </div>
    );
  }
  ...
  const maxSeconds = Math.max(
    ...sortedData.map((day) => day.grand_total.total_seconds)
  );
  const _maxHours = maxSeconds / 3600;
    ...
    <div className="space-y-6">
      <div className="space-y-4">
        {sortedData.map((day) => {
          ...
          const isToday =
            new Date(day.range.date).toDateString() ===
          ...
          return (
            <div key={day.range.date} className="space-y-1">
              <div className="flex items-center justify-between text-sm">
                <span
                  className={`w-32 ${isToday ? 'font-medium text-foreground/90' : 'text-foreground/70'}`}
                >
                  {formatDate(day.range.date)}
                </span>
                <div className="mx-3 flex-1">
                  <div className="h-3 overflow-hidden rounded-full border border-[#3D3D3D]/20 bg-[#1A1A1A]">
                    <div
                ...
                <span
                  className={`min-w-20 text-right text-xs ${isToday ? 'font-medium text-foreground/90' : 'text-foreground/70'}`}
                >
      ...
      {/* Weekly summary */}
      <div className="mt-8 rounded-md border border-[#3D3D3D]/10 bg-[#242424]/30 p-4">
        <h3 className="mb-3 font-sans text-lg text-foreground/80">Summary</h3>
        <div className="flex flex-wrap gap-4">
          <div className="min-w-[180px] flex-1">
            <div className="text-sm text-foreground/60">Total Coding Time</div>
            <div className="mt-1 text-xl font-semibold text-foreground/90">
              {formatTotalTime(
                sortedData.reduce(
...
function ProgressBarChart({ data, chartColors }: ChartProps) {
  ...
  return (
    <div className="space-y-4">
      {sortedData.slice(0, 5).map((item, _index) => (
          ...
          <div className="h-3 overflow-hidden rounded-full border border-[#3D3D3D]/20 bg-[#1A1A1A]">
            <div
              ...
              style={{
                width: `${item.percent}%`,
                backgroundColor:
                  item.color || chartColors[_index % chartColors.length],
              }}
            />
          </div>
...
function _BarChart({ data, chartColors }: ChartProps) {
  // Sort data by percentage (descending)
      ...
      {sortedData.map((item, _index) => {
        ...
        return (
          <div key={item.name} className="flex w-full flex-col items-center">
            <div
              className={`${chartColors[_index % chartColors.length]} w-full rounded-t-md transition-all duration-500 ease-in-out`}
              style={{ height: `${height}%` }}
            />
            <div className="mt-2 w-full truncate px-1 text-center text-xs text-foreground/70">
              {item.name}
            </div>
...
function _TableView({
  ...
  data: Array<{ name: string; percent: number; text: string }>;
}) {
  // Sort data by percentage (descending)
  const sortedData = [...data].sort((a, b) => b.percent - a.percent);
      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="border-b border-[#3D3D3D]/30">
            ...
            <th className="px-4 py-2 text-right font-medium text-foreground/80">
              Percentage
            </th>
            <th className="px-4 py-2 text-right font-medium text-foreground/80">
              Duration
        ...
        <tbody>
          {sortedData.map((item, _index) => (
            <tr
              key={item.name}
              className="border-b border-[#3D3D3D]/10 transition-colors hover:bg-[#242424]/30"
            >
              <td className="px-4 py-2 text-foreground/70">{item.name}</td>
